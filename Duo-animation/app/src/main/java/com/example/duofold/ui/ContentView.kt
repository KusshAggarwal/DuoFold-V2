...
fun ContentView(motionModel: FoldMotionModel) {
    ...
    val hasHingeSensor by motionModel.hasHingeSensor.collectAsState()
    val hingeAngle by motionModel.hingeAngleDegrees.collectAsState()
    ...
    // Real hinge sensor => a genuine center crease (both panels fold
    // symmetrically). Simulated tilt or manual slider => the legacy one-edge
    // "lifting sheet" look, edge chosen the same way it always was.
    val usingRealHinge = effectiveUseSensor && hasHingeSensor
    val hingePosition = if (usingRealHinge) 0.5f else if (hinge >= 0f) 1f else 0f
            ...
            AlertDialog(
                ...
                text = {
                    Column(Modifier.fillMaxWidth()) {
                        Text(
                            if (usingRealHinge) {
                                "Tilt: %.1f°  ·  real center hinge".format(tilt)
                            } else {
                        ...
                        if (hasHingeSensor) {
                            ...
                            Text(
                                if (hingeAngle != null) {
                                    "Real hinge: %.0f°".format(hingeAngle)
                                } else {
                                    "Real hinge sensor detected"
                                },
